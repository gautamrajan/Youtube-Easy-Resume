...
((root, factory) => {
    ...
    root.YouTubeEasyResumeVideoStorage = api;
})(typeof globalThis !== "undefined" ? globalThis : this, () => {
    const SCHEMA_VERSION = 1;
    const SCHEMA_VERSION_KEY = "videoStorageVersion";
    ...
    function createVideoStorage(storageArea, getLastError, now) {
        if (!storageArea) {
            throw new Error("A browser storage area is required");
        }

        const readLastError = getLastError || (() => null);
        ...
        async function initialize() {
            ...
            legacyVideos.forEach(legacyVideo => {
                ...
                const key = getVideoKey(video.videolink);
        ...
        async function getVideo(link) {
            const key = getVideoKey(link);
            if (!key) {
                return null;
            }

            const data = await get(key);
            return hasValidUpdatedAt(data[key]) ? data[key] : null;
        }

        async function getAllVideos() {
            const data = await get(null);
            return Object.keys(data)
                .filter(key => key.startsWith(VIDEO_KEY_PREFIX))
                .map(key => data[key])
                .filter(video => {
                    return hasValidUpdatedAt(video) && getVideoKey(video.videolink);
                })
        ...
        async function removeVideos(videos) {
            const keys = videos
                .map(video => getVideoKey(video && video.videolink))
                .filter(Boolean);
            if (keys.length > 0) {
